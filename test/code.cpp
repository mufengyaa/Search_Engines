...
private:
    ...
    getTimeoutForTask(const std::string &taskType)
    {
        ...
        else if (taskType == ns_helper::TASK_TYPE_PERSIST_INDEX) // ✅ 修复这里
        {
...
void test_cancel_task(exec_context ctx)
{
    ...
    for (int i = 0; i < 50; ++i)
    {
        if (ctx.canceled())
        {
            std::cout << "Task detected cancellation, exiting early.\n";
            return;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        std::cout << "Task working... step " << i << "\n";
    }
...
int main()
{
    FixedThreadPool fp(2, 10, FixedThreadPool::RejectionPolicy::BLOCK);

    auto future = fp.submit("search", test_cancel_task);
    ...
    {
        std::cout << "Task threw exception: " << e.what() << "\n";
    }
